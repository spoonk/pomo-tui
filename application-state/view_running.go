...
func (m Model) runningSessionView() string {
	...
	d := m.sessionTimer.GetUnpausedDuration()
	...
	timerText += d.String()
	timerText += " / "
	timerText += formatDuration(m.sessionTimer.GetTimeLimit().Round(time.Second))
	...
	breakText := fmt.Sprintf(" - (break: %s)", formatDuration(m.breakTimer.GetTimeLimit()))
